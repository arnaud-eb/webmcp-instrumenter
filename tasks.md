...
> **⚠️ SUPERSEDED for feature build.** The authoritative, dependency-ordered task
> list is now `specs/001-webmcp-instrumenter/tasks.md` (generated by
> `/speckit-tasks` from the spec + plan + contracts). This root file is retained
> only for the **Phase 6 real-deploy operational tasks** (Shopify Partners store,
> origin-trial token, organic-traffic window) that are out of the code build's
> scope. For implementation, follow the feature `tasks.md`.
